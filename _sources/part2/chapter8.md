...
The `layout` now has another dictionary `'title': {'text': 'My first plotly figure'}`, and `data` has a list with one trace element.

Notice how we've used both `fig.show` and `fig.show()`. The former let's you study the content of the figure object while the latter displays a visual representation of the figure.

### 3. Figure object methods
...
#### 5. Easy data handling with Plotly Express
...
#### 6. Inspect the structure of a `px.line()` figure
...
#### 7. Plotly Express marker symbols
...
Even though these techniques seem very different, `px.line(markers = True)` and `fig.update_traces(mode = 'markers+lines')` do the very same thing to the underlying structure of the `fig` object, namely setting:

    'mode': 'markers+lines'

for the all traces contained in `data` in the figure dictionary.
...
### 8.1 Visualizing multiple dimensions of a dataset
...
### 8.3 Adding another dimension to a dataframe of a long format
