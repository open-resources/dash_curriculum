...
````
2.  Incorporate the callback we just built into the app code included in the "Bring everything together". The new app, besides allowing to change the title text and size, should also allow to change the title colour based on the `RadioItems` component.
````{dropdown} See Solution
    :container: + shadow
    :title: bg-primary text-white font-weight-bold
